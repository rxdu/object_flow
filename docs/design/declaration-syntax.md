...
| 23 | Deciding D345's question (ADR-0109): a condition every object in a state must meet is an invariant over that state, and the publish report lists what each creation past its lifecycle's first state skips, computed by the checker and verified where a document quotes it. Writing the journey's invariants found that nothing recorded a unit's manufacturer serial or photos after creation (D379), and a planted mistake found that check 19 ignored a bare state compared with the object's own (D380) |
| 24 | The author's decision to deploy the engine as an internal service, leave authentication and the mapping of roles to capabilities to upper layers, and declare each actor's kind with its type (ADR-0110). The `actor` marking names a kind, check 62 enforces it, and the example's agents become their own type |

## 1. Shape of a file

A file is a module, extension `.of`. Declarations may appear in any order; forward references are fine. `#` begins a comment.
...
capability INVENTORY_CREATE, INVENTORY_RETIRE, INVENTORY_ASSERT,
           ...
           PO_EDIT, CLAIM_EDIT, SERVICE_EDIT, SERVICE_ASSIGN, PDI_RECORD, AUDIT_VIEW,
           AGENT_ADMIN
...
Eleven top-level forms: `module`, `use`, `capability`, `category`, `enum`, `sequence`, `evaluator`, `machine`, `type`, `observation` (§6.8) and `metric` (§6.9). One more line states a rule about requests rather than about any type, and like the vocabularies it applies across the closure (ADR-0103):

```text
requests by agent require version, key
```

**`requests by <kind>, … require <field>, …`** refuses a request by an actor of a named kind (`human`, `agent`, `service`) that lacks a named field: `version`, its `expected_version`, and `key`, its `idempotency_key`. The refusal names a generated clause, `versioned` where the request names an existing object and carries no version, `keyed` where it carries no key, each with remedy `self_serviceable`, and is counted like any refusal. It applies to what an actor sends — transitions, recordings, labels and a proposal's approval, though not the recorded request an approval executes — after the replay test and before the version test (DESIGN.md §6); `publish` keeps its own required version, and several lines add up. An agent's tool schema lists the required fields, and `check` and `availability`, which answer before a request is sent, do not evaluate the two clauses (ADR-0103).
...
**Seven capabilities are built in** the same way, and gate the built-in types' transitions: `OF_DRAFT_CHANGE` and `OF_APPROVE_CHANGE` for a flow change, `OF_LABEL` for labelling, `OF_ERASE` for erasing one observation, `OF_IMPORT` for the import, `OF_MAINTAIN` for maintenance and `OF_SUBSCRIBE` for creating a subscription. The rule set prints them with the types that use them, and a deployment's authentication emits them like any capability (ADR-0097, ADR-0105). They, the built-in types, the `label` kind, the `closed` category and the standard metrics of §6.11 are **declaration version 0**, which a store is created with, and each later version records the built-in module it was published with, so an engine release changes them in a store only through a publish (DESIGN.md §5.9, ADR-0105).
...
| Marking | Meaning |
|---|---|
| `identifier from <seq> [scoped by <ref or indexed attr>] format "<pattern>"` | Minted during the creation. A scope names a reference or an indexed attribute the creation writes, and the mint happens after those writes and before every other outcome step, which is the order that lets a serial be scoped by the model it is created under. An attribute is allowed because a tenant is often an identifier mirrored from another system rather than an object here. The pattern holds literal text and placeholders: `{n}`, the number; `{n:<width>}`, the number zero-padded to at least that many digits; `{<attr>}` or `{<ref>.<attr>}`, a `string` or enum attribute of the object or of one a reference names, which every creation must write before the mint, as a scope must, and which an enum fills with its member's name; and `[ … ]`, a segment left out whole when a placeholder inside it is absent, which is the only place an optional attribute may be named. So `format "RBT-[{model.maker_code}-]{n:6}"` mints production's serials, the maker's segment dropped where a model has no code (ADR-0103) |
...
| `actor <kind>` | On an `identity` attribute: this attribute holds the id of the actor the object stands for, and `<kind>` — `human`, `agent` or `service` — is the kind of every actor of this type. A request's actor is the live object whose actor identity its descriptor names, and its kind is this one, never the descriptor's (ADR-0110). An `assignee` reference to this type compares against it (§6.10, checks 59 and 62) |
...
An invariant may read the object's own `state`, and that is how a condition every object in a state must meet is written: `invariant labelled: state != AVAILABLE or label_printed_at is not null` binds every route into `AVAILABLE`, where a guard binds only the transition it is on (ADR-0109).
...
**The erasure is reached, not written.** The subject's `erase` runs `forget` on every observation in its collection, corrected ones included, as it redacts its labels' notes, without a step saying so, and check 39 counts an observation part as reached. An actor holding `OF_ERASE` may also request `forget` on one observation, which is how a personal value recorded about someone other than the subject is removed without erasing the subject (ADR-0096, ADR-0100, PRD D8).

**Labels.** Every type carries labels without declaring any. A label is an observation of the built-in kind `label`, with a `name`, normalised to lower case and trimmed; an optional `note`, which is personal (ADR-0078's limit on free text); and `subject_state`, the subject's state when it was applied. The name is vocabulary, not personal, and erasure does not reach it, which is the known limit on free text (DESIGN.md §8, ADR-0106). Its occurred time is its recorded time, since a label records that someone noticed something then; where when a thing happened matters, the fact is an observation kind with `occurred within` (ADR-0106). Its `forget` redacts the note, so a note is erasable on a type that declares no `erase` of its own. Labelling requires `OF_LABEL`, and a type may narrow it with a condition over `actor`, conjoined with the capability:
...
type Agent version 1 {
  tracking record
  states   ACTIVE category live, REVOKED category closed terminal
  attr     key_id identity actor agent unique
  attr     name   string
  create issue -> ACTIVE accepts key_id, name { require may: actor.has(AGENT_ADMIN) because delegable }
  do revoke ACTIVE -> REVOKED { require may: actor.has(AGENT_ADMIN) because delegable }
}
...
**Each kind of actor is its own type** (ADR-0110). `User` holds people and `Agent` holds agents, each naming its kind on its actor identity, so a request's kind is its type's and no descriptor can claim another. The first consumer keeps its agents as users with the role `AGENT`; the port makes each one an `Agent`, so `UserRole` has no such member, and an agent cannot be named engineer-of-record at all, since `engineer` references a `User`. Where a deployment's own user system owns its roles, as the first consumer's does, `User` is externally owned and kept in step by a sync (ADR-0080), whose requests to `add` and `leave` are gated on a capability only the sync holds; this example declares them locally, for brevity.
...
The places an expression is evaluated each need an answer, and they are not the same answer:
...
- `*` and `/` take a scalar — `int` or `decimal` — on at least one side, except the quotient of two like quantities below. With one non-scalar operand the result is that operand's type, scale preserved. With two scalars, `int * int` is `int`, `decimal(p1,s1) * decimal(p2,s2)` is `decimal(p1+p2, s1+s2)`, and `int * decimal(p,s)` is `decimal(p+19, s)`. `money * money` is an error, as is `duration * duration` (check 24).
- `/` takes its scalar on the **right** and yields the left operand's type, so `money / int` is money and `int / money` is an error (check 24). **A quotient of two like quantities** — two `duration`s, or two amounts of one currency — is a `decimal(19,6)`, rounded half to even, so a utilisation or a share of revenue can be declared; quantities of different kinds, or of different currencies, remain an error (ADR-0103). **`int / int` is `int` and discards the remainder**, which is stated because it is a trap in any domain where the quotient is money; divide a `decimal` if you need the fraction. Division by zero yields `unknown` (§8.2), never an error, so a guard containing one is unsatisfied rather than a failed request.
...
requests by <kind>, … require <field>, …                         # §1; kind human|agent|service, field version|key
...
| 10 | Whether a `counter` should hold `money` and go negative | **Deferred.** Non-negativity does move out of `counter` and into an invariant. *(Corrected 2026-09-24: this answer first said the counter is unexercised by the first consumer, which "stores no stock level". It keeps one for bulk accessories, counted down when a service consumes them (`wr:app/services/service_service.py:751-773`), which the design models as a `tracking quantity` lot with a counter and `nonneg`, so the counter is exercised; whether it should hold money stays deferred, D279, ADR-0103.)* | ADR-0072 |
...
Two answers moved because of the survey rather than the argument. Per-attribute confidentiality looked likely and turned out to have no need at all. The money counter turned out to rest on a mechanism the first consumer was then thought not to use. *(Corrected 2026-09-24: it keeps a stock level for bulk accessories, question 10, D279.)*
