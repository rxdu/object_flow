...
A single-pass read of the whole record by one reviewer — the model, all 75 decisions, the seven design documents, the five case studies, the walkthrough, the edge cases and the status files — after the six documents of 2026-09-09 were written, with three claims probed by running something rather than reading. The findings cluster where `docs/LESSONS.md` predicts: at the seams between the newest documents and the model. Entries are ordered by consequence. D187 and D188 were repaired the same day by ADR-0076 and nine more by ADR-0077; D190, D193 and D194 are **open** because they need the author's ruling; D201, found by the repair, was closed the same day.
