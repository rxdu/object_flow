...
| [D201](#d201) | ADR-0075 breaks a cycle by the optionality of its edges, which answers the import question and not the stage question | Open |
---
...
A single-pass read of the whole record by one reviewer — the model, all 75 decisions, the seven design documents, the five case studies, the walkthrough, the edge cases and the status files — after the six documents of 2026-09-09 were written, with three claims probed by running something rather than reading. The findings cluster where `docs/LESSONS.md` predicts: at the seams between the newest documents and the model. Entries are ordered by consequence. D187 and D188 were repaired the same day by ADR-0076 and nine more by ADR-0077; D190, D193 and D194 are **open** because they need the author's ruling, and D201 was found by the repair.
...
**Open.** Amend ADR-0075's derived paragraph and the cutover document's §2 to say that a cycle is one stage regardless of optionality, or supply the argument by which an optional edge lets a legacy referrer live without its migrated referent. The paragraph is derived rather than the author's ruling, but it sits inside an ADR the author accepted, so the author should see the change.
