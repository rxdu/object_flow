...
| [D187](#d187) | On SQLite the sequence mint deadlocks against the request that needs it | Open |
| [D188](#d188) | A retry carrying both its idempotency key and its `expected_version` is refused as `stale`, not replayed | Open |
...
A single-pass read of the whole record by one reviewer — the model, all 75 decisions, the seven design documents, the five case studies, the walkthrough, the edge cases and the status files — after the six documents of 2026-09-09 were written, with three claims probed by running something rather than reading. The findings cluster where `docs/LESSONS.md` predicts: at the seams between the newest documents and the model. Entries are ordered by consequence. All fourteen are **open**; each says what would close it, and the three that need the author's ruling say so.
...
**Open.** Either the sequence table lives in a separate database file on SQLite, or every mint a request will need is taken before its transaction opens — possible for a creation whose scope comes from an input, not for one inside a cascade. The first is the smaller change. Either way the verified paragraph should name the two-connection probe. Related: D181, whose fix this is.
...
**Open.** Swap steps 2 and 3, and say that a replay returns the recorded verdict whatever version the retry supplies. `library-api.md` §3's description of `expected_version` and `idempotency_key` should say which wins.
