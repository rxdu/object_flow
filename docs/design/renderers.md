...
- **Localisation.** Guard sentences are rendered from expressions into English here. Nothing says whether that rendering is a table the deployment supplies, and a consumer with a non-English UI will ask.
- **Whether a rule set diff is generated or textual.** Byte-identical rendering makes `diff` work; a semantic diff that says "guard `settled` was added to `complete_sale`" would be better and is more work.
