...
- **Recording is validated** (D12). An observation kind's fields are typed, a numeric field may state its unit (`unit "V"`), which the rule set prints and a read returns beside the value — units are not converted or checked in arithmetic — and a kind may declare invariants over its own fields — a voltage within a range — checked when it is recorded like any local invariant. Who may record it is its `recorded by` guard.
...
| UC-12 alerts | flags on metrics, and per-object derived attributes queried by `query` (§3.4); "order ready", an event rather than a condition, by a subscription to the transition that fills the last slot | passes; sending is the consumer's |
