...
**Amended 2026-09-25 for ADR-0110:** the first consumer's backend becomes a client of the engine's service and holds none of the store's database credentials. Its users are ported as `User` objects and its agents, users with the role `AGENT`, as `Agent` objects, each type declaring its actors' kind; its roles stay in its own user system, which keeps `User` in step by a sync (ADR-0080) and maps roles to the capabilities it sends.
