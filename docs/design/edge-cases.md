...
- **Atomicity across ObjectFlow and an external system.** Not covered. A transition and a call to Xero, a payment gateway or a mail server cannot commit together. The design's answer is an externally owned type plus idempotency: the external system's record is an ordinary type here with an external identifier, an upper-layer application performs the external action on observing the event and reports back with the external event id as idempotency key (ADR-0008, ADR-0014, ADR-0080). The window between the two is real and visible in the record.
- **The clock.** Covered with caveat. `now` in a guard is the store's injected clock, read once per request and recorded in the read set (ADR-0077, ADR-0088), not the caller's clock; a guard such as `end_date <= now` is evaluated at that instant, and the harness makes it deterministic by injecting its own. A caller that reasons about time from its own clock can be refused by a few seconds either way. *(Corrected 2026-09-24, D324: this entry said `now` was the database's transaction time, evaluated at commit, which the injected clock replaced.)*
- **Several ObjectFlow stores.** Not covered as one system. Each store guarantees its own data; references across stores are external identifiers, and no invariant spans them.
...
- **Grouped aggregation.** *Reversed in part by ADR-0084, 2026-09-23.* The metric form groups and windows, and a guard reads one metric value by binding its dimensions (DESIGN.md §5.12), which is how PRD C3 is met. What stays out is grouping inside a guard's own expression: a guard that appears to need it usually wants a relationship aggregate on the other end, or a declared metric (ADR-0047). *(Corrected 2026-09-24, D324.)*

## From customer records (iteration 3)

- **Unmerge.** Not built in. A merge is a supersession plus re-pointed links, each recorded with its previous target; a caller can reverse it from history by creating a new record and re-pointing back, but the original id does not return to life, because a superseding state is terminal.
...
- **Four-eyes across different objects** (the same person may not approve both the request and its invoice). Covered only where the two objects are related by a declared relationship the guard can traverse; otherwise it is an upper-layer application's rule.
...
- **Editing a recurring series after instances exist.** Upper-layer logic: instances are objects the application created; which to change is a selection.
- **Waitlist promotion.** Upper-layer logic: "the first in the queue" is a selection with an ordering the language does not express.
- **Business hours and local time.** Not covered. Timestamps are absolute; calendar arithmetic is a caller's input.
- **Substitution bookings** ("any robot of this model"). Upper-layer logic: choosing the unit is a selection; once chosen, the booking is ordinary.
...
- **Exact allocation of an amount into parts that must sum back.** Not expressible, deliberately. Who absorbs the remainder is a decision rather than a calculation, and ADR-0007 puts it with the upper layer.
...
- **Text that must match a pattern** — a Jira key, a link's scheme — **or be trimmed**. Not covered, by decision (ADR-0103). A pattern's meaning differs between Python and each backend's SQL, so it would make a rule's meaning depend on the backend. The first consumer's schema checks the format and turns blank input into absence at its edge, or an evaluator asks the system the text refers to. `length` bounds text in the store.
...
- **Who may see who acted.** Covered as the first consumer does it: a reader who may see a record sees who acted on it, and the cross-record trail its administrators read — every entity's activity, one user's activity — is the export and a subscription's `pull`, served from routes the deployment authorises. A rule withholding who acted from ordinary readers was considered and withdrawn: it was stricter than production and left the assignment metrics incomplete for them (ADR-0103, D285).
- **A list filtered on a status derived from other objects**, such as an order's status from its units. Not covered as a query filter: a derivation that reads other objects cannot be indexed (check 46), and a stored copy written as a side effect of other objects' transitions would be a second write path. Where the status gates a transition, `available` answers it in one query (ADR-0048). Otherwise the reader filters the page, which is a scan; none of the questions PRD N6 and UC-20 name needs it, but ADR-0104 §4 counts any need met by scanning as a gap in the core, so `TODO.md` records it as one; a status that needs to be a filter can meanwhile be modelled as the object's own state. *(Corrected 2026-09-24, D343: this entry treated the reader's filter as the answer, which revision 4 of the PRD made a gap.)*
...
- **Notes attached to eleven kinds of record.** Open, and the options are stated once, in [`first-consumer-cutover.md`](first-consumer-cutover.md) §1. *(Corrected 2026-09-24, D324: this entry and the cutover plan listed different options.)*
