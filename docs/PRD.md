...
| **Engine** | ObjectFlow itself, also called the core or the store. It is deployed as an internal service, one per store, and inside the service the core is a library (N2, N7) |
| **Actor** | A person, an agent or a service that makes requests. Each is an object of a type the deployment declares as holding actors of one kind, and a request names its actor by that object's identity (T6) |
...
| T2 | The threat model is mistakes, not malice: actors are trusted and fallible, and one holding the store's database credentials, or network access to the engine's service, is out of scope | Must | Carried; the extension to network access to the service is Inferred from N7 | — assumption |
...
| T6 | The kind of every actor — human, agent or service — is declared with the type that holds it, and changes only by publishing (F5). A request names its actor, the engine takes the actor's kind from that declaration, and a request whose actor is not a live one is refused | Must | Said, 2026-09-25: "define the actor kind along with the objects, flows etc. and we rely on the upper layer to handle the role mapping"; the refusal of an actor who is not live is Inferred | UC-16, UC-18, UC-22 |
...
| N7 | The engine is deployed as an internal service, one per store, that every caller reaches through one interface: upper-layer applications, the first consumer's own backend and agents alike. No caller embeds the core or holds the store's database credentials, and inside the service the core stays a library (N2). Authentication, and which capabilities each actor holds, are the upper layers'; the engine authenticates nothing. What a capability permits, and who may see what, stay rules the engine declares and enforces on every request (F1, T5) | Must | Said, 2026-09-25: "deploy the engine as a service rather than a library so that upper layers rely on the same interface to interact with the flow", "treat the engine as an internal service", "leave the authentication to the upper layer completely", "rely on the upper layer to handle the role mapping". That no caller embeds the core or holds its credentials, and that the rules on capabilities stay in the engine, are Inferred | UC-20, UC-22 |
...
**UC-22. One door, and a kind nobody can claim.** The first consumer's backend, the operations application and an agent gateway all call the engine's service, and none holds the store's database credentials. The gateway has a bug: it passes each agent the capabilities of the person it works for. An agent then asks to be named engineer-of-record of a service job, and later a user who has left the company acts through a session nobody revoked. *Acceptance:* every request reaches the store through the service and is recorded with the actor it named; each actor's kind is the one its declared type gives, whatever the gateway sent, so the agent's requests are held to the rules for agents — a version and a key on every request, and never engineer-of-record — and counted as an agent's; the departed user's request is refused; and changing the kind of a type's actors is a publish.
...
**Revision 8, 2026-09-25**, at the author's direction, after comparing an embedded library with a service: "deploy the engine as a service rather than a library so that upper layers rely on the same interface to interact with the flow"; "treat the engine as an internal service"; "leave the authentication to the upper layer completely"; "define the actor kind along with the objects, flows etc. and we rely on the upper layer to handle the role mapping".

- **Where the engine runs was a design choice nobody had made.** DESIGN.md allowed both an embedded library and a service. N7 makes the service the deployment, so every caller uses one interface and none holds the store's database credentials, while N2's library stays inside it.
- **Who proves identity, and who assigns roles, were unstated.** N7 gives both to the upper layers. What a capability permits, and visibility, stay the engine's rules, since F1, T1, T5, F7 and D12 are Musts and the first consumer's most repeated rule is a permission check.
- **Kind was a claim on every request.** With authentication upstream, a rule depending on an actor's kind would have trusted each descriptor's word for it. T6 declares the kind with the type that holds the actor, and UC-22 tests it with a gateway that gets an agent's capabilities wrong.
- **T2 now names network access to the service** beside the database credentials, since the service authenticates nothing.
