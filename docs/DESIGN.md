...
**Status: under review.** Every finding of every review is recorded in [`design/defects.md`](design/defects.md), 382 entries and five cosmetics; 382 are closed and 0 are open. The PRD is at revision 8, with no revision awaiting the author.
...
This document is the single description of the **model**: what an object is, what a transition guarantees, how a request executes, what the store refuses. The **language** those things are written in belongs to [`design/declaration-syntax.md`](design/declaration-syntax.md), which owns every grammar, every spelling and the sixty-two publish checks.
...
Every request carries an **actor descriptor** that the upper layer's authentication produces: the actor's id, an optional principal, and a set of opaque capabilities the upper layer maps from its own roles (ADR-0025, ADR-0079, ADR-0110). ObjectFlow authenticates nothing, and validates the descriptor's shape, not its truth. **An actor's kind is declared, not sent.** A type that holds actors marks one `identity` attribute `actor human`, `actor agent` or `actor service`, and a request's actor is the live object whose actor identity the descriptor names, with its type's kind. A request whose actor, or principal, names no live actor is refused, naming the generated clause `actor_live`, remedy `delegable` (§6). An actor identity names one actor across every such type and is never reused, so history's attribution stays unambiguous. A guard reads **four** members of it — `.id`, `.kind`, `.principal` and `.has(<capability>)` — and nothing else: the descriptor carries no declared types, so a guard over any other value could be neither checked at publish nor recorded in the log. Guards read it, and every event records it. Delegation arrives in the descriptor as capabilities with `principal` set to the delegator. Attenuation is a capability where the fact is one of a few fixed values, and otherwise an object the guard reads — a `Delegation` with a limit and a lifecycle — which is recorded, revocable and the pattern ADR-0036 gives for governed delegation. Where a guard needs a referenced person, that person is an ordinary object.
...
1. Resolve the actor, and the principal if the descriptor names one: each must be a live object of a type that marks an `actor` identity, and its kind is that type's (§5.8). Otherwise refuse, naming `actor_live` (ADR-0110). Then, if the type declares visibility and the actor cannot see the object, refuse as `not found`.
...
**The service authenticates nothing** (ADR-0110, PRD N7). A caller that can reach it can name any live actor and claim any capability, which PRD T2 places out of scope, as it places the database credentials. So a rule reading a capability is only as good as the upper layer's mapping of roles to capabilities, and the record keeps every descriptor, so a wrong one is attributable. An actor's kind is not claimed at all: it is declared with the type that holds the actor.
...
| **Actor** | Whoever makes a request: a live object of a type that marks an `actor` identity, whose kind that type declares. A request names it in its descriptor — id, principal, capabilities — and a guard reads `.id`, `.kind`, `.principal` and `.has(…)`; a fact about an actor is a capability or an object (ADR-0079, ADR-0110). |
