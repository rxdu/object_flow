...
**Status: under review.** Every finding of every review is recorded in [`design/defects.md`](design/defects.md), 268 entries and five cosmetics; 268 are closed and 0 are open. The author accepted ADR-0081 to ADR-0096 on 2026-09-23 — ADR-0087 to ADR-0096 decided at the author's direction to cover every PRD requirement ([`design/traceability.md`](design/traceability.md)), and accepted after each decision was checked against this design; ADR-0097 to ADR-0101, decided at the author's direction the same day from a five-slice review of the whole record and its verification, await acceptance; the author has ruled on ADR-0065 to ADR-0073; ADR-0019 to ADR-0064, ADR-0074 to ADR-0080 and the six implementation documents of 2026-09-09 await review, ADR-0078 to ADR-0080 first, being the three decided at the author's direction rather than by the author.
...
Every type gets **standard metrics** without declaring any. They are declarations, given in `design/declaration-syntax.md` §6.11 with the names `metric()` reads them by, and checked like any other (ADR-0098). Open and finished are read from the one category with a meaning of its own, `closed`, which every vocabulary has: work is open until it enters a `closed` or a terminal state, and completes when it first does (ADR-0096). None counts the import's events, which are the port and not the flow; a span still open on a finished object stops at its completion, so an engineer still named on a closed job accrues no time; each can be split by version and actor kind; and two name the objects, so the jobs that changed hands more than once can be listed (ADR-0101).
...
A type with an assignee also gets the metrics of §5.13. All of them are listed with the type, like any declared metric, so a person reading the rule set and an agent calling `metric()` see the same definitions. **A reader reads a metric as a guard does**: `metric()` binds dimension values, applies a window and a filter, and aggregates over any dimension left unbound, so a screen, an agent and a rule read one value from one definition (ADR-0098). A derived attribute a metric row reads is evaluated under the reader's visibility, and the types it reads count toward whether the result is complete.
...
| **metric**(name, bind?, over?, filter?, cursor) | A declared or standard metric computed over the rows the reader may see, with dimensions bound and a window applied as a guard's reference does, narrowed by the filter before it aggregates, and aggregated over any dimension left unbound: each group with its dimensions, its value and the flags that hold, in UTC calendar time, and whether the result is complete (§5.12, ADR-0084, ADR-0096, ADR-0098) |
...
| **import_batch**(batch, dry_run?) | Writes a port through the built-in assertion, for `OK_IMPORT`, into a mirror or a type no ordinary request has written; every event marked `imported` (§11, ADR-0100) |
| **maintain**(task) | Prunes attempt rows past the retention the store was built with, keeping their counts in the rollup, or moves events to the archive the log's reads include, for `OK_MAINTAIN`; changes no governed state or history, and refuses a prune inside the retention (ADR-0100, ADR-0101) |
