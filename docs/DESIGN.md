...
| **import_batch**(batch, dry_run?) | Writes a port through the built-in assertion, for `OK_IMPORT`, into mirrors and the observations and labels on a mirror's objects; never rewrites what an erasure removed; every event marked `imported` (§11, ADR-0100, ADR-0101) |
