...
- **Date:** 2026-09-08
- **Refined by:** ADR-0059 — both ends of a relationship are declared, and cardinality decides which one stores the value; ADR-0060 — twelve decisions from the iteration-11 syntax review; ADR-0061 — ten decisions from the payments-ledger review; ADR-0066 — a cascade clause carries arguments. ADR-0082 — an observation is a born-final part, recordable directly and implied `survives`. ADR-0100 — a re-parent carries a generated guard, `whole_open`: the destination whole is not in a terminal state.
