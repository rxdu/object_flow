...
3. **The read surface provides an availability query:** given a type and a transition name, the objects for which that transition is currently available, or available-with-input, for a given actor. Deferred external guards (ADR-0008) are not evaluated by this query and the result says so.
...
- Alerts that are not transitions — "lease overdue" — are derived attributes (`state == OUT and expected_return < now`) the automation layer polls through the ordinary read surface.
