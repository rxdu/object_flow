...
- **Date:** 2026-09-08
- **Refined by:** ADR-0083 — intervals are an index the log rebuilds, and refusals go to an attempt log that is not history. ADR-0088 — every event records the read set of the rules evaluated for it.
...
1. **Current state is stored.** Each object is one row: id, type and declaration version, state, attributes, `version`, and the id of the last event that changed it. Guards, derived attributes, visibility predicates, type-scan invariants and the read surface read this row. Indexes are declared on it.
...
3. **Provenance attaches to the event**: actor and principal (ADR-0025), `context` — a free-form string the caller supplies naming the route a request came by, such as `form:contact-us` or `import:batch-42` — cause (the parent event for a cascade, ADR-0019; the source event for an event-driven request, ADR-0014), declaration version (ADR-0027), and a `source` of `observed` (a transition produced it), `asserted` (import or override), `migrated` (ADR-0027), `corrected`, or `erased` (ADR-0031).
