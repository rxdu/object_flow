...
- **Date:** 2026-09-09
- **Refined by:** ADR-0083 — `state_source` gains `imported`, kept apart from `asserted`. ADR-0089 — the settled position becomes a settled cursor; ADR-0091 — the connection source joins the injected dependencies; ADR-0092 — `Stale` carries its cause. ADR-0105 — `deliver_as` is withdrawn with push delivery; a subscription names the one reader who may pull and acknowledge it.
