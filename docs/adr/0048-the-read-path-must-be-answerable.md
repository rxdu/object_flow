...
1. **A per-attribute last-written index.** Alongside each object row the store maintains, per attribute, the position of the event that last wrote it. It is derived from the log and updated in the writing transaction, so it can never disagree. `changed_since(attributes, event)` becomes a constant-time comparison instead of a history scan.
