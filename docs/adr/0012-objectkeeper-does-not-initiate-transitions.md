...
- **The read path becomes load-bearing.** Detecting that nothing happened is still real operational work; it moves upstairs, and the upper layer needs queries such as "which cases are in `awaiting_customer` and have not moved in four hours". That capability is now required, not optional.
- **ADR-0010's property acquires an exception.** Timing rules like "escalate after four hours" live in the automation layer, in its language, free to drift from the object type they concern. Whether ObjectKeeper should carry such rules as *inspectable but unexecuted* metadata — preserving one source of truth without a scheduler — is recorded as an open question in TODO.md.
