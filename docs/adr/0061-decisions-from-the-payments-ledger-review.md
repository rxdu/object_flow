...
- Four findings are **not** resolved and are open questions 9 to 12: money with a runtime currency, a counter that holds money and may go negative, an `only via` form that names a family, and whether `serial` and `quantity` are the right two tracking modes. The last reopens ADR-0050 and the first three are each a model change, so all four are the author's to rule on.
