...
3. **An invariant that traverses relationships may traverse only ones with declared inverses.** This makes the affected set computable by reverse traversal from any written object. A declaration whose invariant traverses a one-way path is rejected at publish, naming the relationship that needs an inverse. *Amended by ADR-0052: an invariant may instead scan a type, provided its predicate is **symmetric**, which makes the affected set the same predicate read from the other side. The two forms are the only ones permitted; the original wording read as a blanket prohibition on type scans, which would have excluded the booking overlap invariant.*
