...
Every request carries an **actor descriptor** that the consumer's authentication produces and ObjectFlow accepts as given:
...
- ObjectFlow validates the descriptor's shape, not its truth. Authentication, key management, roles and how capabilities are computed belong to the consumer.
...
### ObjectFlow manages users, roles and keys
