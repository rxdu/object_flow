...
- [ ] **Compute the first consumer's stage order.** The reference graph is in that repository's schema, so the order and its cycles are derivable. ADR-0075 forces referrers before referents; what is not known is whether the graph has cycles, and each one is a stage that must move at once. This is the first thing the cutover plan needs.
